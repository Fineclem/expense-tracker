...
const JWT_SECRET =
  ...
  "fineclem_expense_tracker_secret_key_2024_production";

// Log JWT secret status (without revealing the actual secret)
console.log("JWT_SECRET loaded:", JWT_SECRET ? "Yes" : "No");
console.log(
  "JWT_SECRET source:",
  process.env.JWT_SECRET ? "Environment variable" : "Default fallback"
);
...
app.get("/api/expenses/list", authMiddleware, (req, res) => {
  const userId = req.user.id;
  db.all(
    "SELECT id, amount, category, note, date, created_at FROM expenses WHERE user_id = ? ORDER BY date DESC, id DESC LIMIT 100",
    [userId],
    (err, rows) => {
      if (err) return res.status(500).json({ error: "DB error" });
      res.json(rows);
...
app.get("/api/budget", authMiddleware, (req, res) => {
  ...
  db.get("SELECT * FROM budgets WHERE user_id = ?", [userId], (err, row) => {
    ...
    if (!row) {
      ...
      stmt.run(userId, 1000.0, 250.0, 35.0, function (err) {
        if (err) {
...
app.put("/api/budget", authMiddleware, (req, res) => {
  ...
  db.get("SELECT id FROM budgets WHERE user_id = ?", [userId], (err, row) => {
    ...
    if (row) {
      ...
      const stmt = db.prepare(
        'UPDATE budgets SET monthly_budget = ?, weekly_budget = ?, daily_budget = ?, updated_at = datetime("now") WHERE user_id = ?'
      );
      stmt.run(
        monthly_budget,
        weekly_budget,
